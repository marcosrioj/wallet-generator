...
test("endereco BTC mainnet comeca com bc1", () => {
  const batch = generateBitcoinAddresses({
    count: 1,
...
test("derivation paths Bitcoin seguem BIP84", () => {
  ...
  assert.equal(bitcoinDerivationPath(2), "m/84'/0'/0'/0/2");
});

test("validacao rejeita formatos Bitcoin que nao sejam mainnet bc1", () => {
  assert.equal(validatePublicAddress("bc2qxy2kgdygjrsqtzq2n0yrf2493p83kkfjhx0wlh").valid, false);
});
